...
def parse_args():
    ...
    parser.add_argument('-Lsub',metavar='Lsub',dest='Lsub',type=int,default=4,help='set Lsub')
...
## order: (c_k_up, c_k+Q_up, c_k+2Q_up, c_k+3Q_up, c_k_dn, c_k+Q_dn, c_k+2Q_dn, c_k+3Q_dn)
    ed0 = tb_ene(kx,ky)+0.5*U*n
    ed1 = tb_ene(kx+Qx,ky+Qy)+0.5*U*n
    ed2 = tb_ene(kx+2*Qx,ky+2*Qy)+0.5*U*n
    ed3 = tb_ene(kx+3*Qx,ky+3*Qy)+0.5*U*n
    dlt0 = -0.5*U*m*(1.0-1.0*1j)
    dlt1 = np.conjugate(dlt0)
    ham0 = np.array([\
        [ed0,  0.0,  0.0,  0.0,  0.0,  dlt0, 0.0,  dlt1],\
        [0.0,  ed1,  0.0,  0.0,  dlt1, 0.0,  dlt0, 0.0 ],\
        [0.0,  0.0,  ed2,  0.0,  0.0,  dlt1, 0.0,  dlt0],\
        [0.0,  0.0,  0.0,  ed3,  dlt0, 0.0,  dlt1, 0.0 ],\
        [0.0,  dlt0, 0.0,  dlt1, ed0,  0.0,  0.0,  0.0 ],\
        [dlt1, 0.0,  dlt0, 0.0,  0.0,  ed1,  0.0,  0.0 ],\
        [0.0,  dlt1, 0.0,  dlt0, 0.0,  0.0,  ed2,  0.0 ],\
        [dlt0, 0.0,  dlt1, 0.0,  0.0,  0.0,  0.0,  ed3 ],\
        ])
    ene0, vec0 = np.linalg.eigh(ham0)
...
#----
            if \
                kx - ky < + np.pi*1.0/2.0 - eps_RBZ and \
                kx - ky > - np.pi*1.0/2.0 + eps_RBZ and \
                kx + ky < + np.pi - eps_RBZ and \
                kx + ky > - np.pi + eps_RBZ and \
                ky < 0 and ind < Ne_RBZ:
#
...
#----
            if \
                ...
                kx + ky < + np.pi + eps_RBZ and \
                kx + ky > - np.pi - eps_RBZ ) and \
                ( np.abs(kx - ky - np.pi*1.0/2.0) < eps_RBZ or \
                np.abs(kx - ky + np.pi*1.0/2.0) < eps_RBZ or \
                np.abs(kx + ky - np.pi) < eps_RBZ or \
                np.abs(kx + ky + np.pi) < eps_RBZ ) and \
                ky < 0 and ind < Ne_RBZ:
#
                list_RBZ_kx.append(kx)
                list_RBZ_ky.append(ky)
...
def calc_mean_field(Lx,Ly,Lsub,Lspin,LRBZsize,BCx,BCy,filling_numer,filling_denom,list_kx,list_ky,U,num,Qx,Qy):
    ...
    n = (num[0,0]+num[1,1]+num[2,2]+num[3,3]+num[4,4]+num[5,5]+num[6,6]+num[7,7]).real
    m = 0.5*(\
        +0.5*(1.0-1.0*1j)*(num[0,5]+num[1,6]+num[2,7]+num[3,4]+num[4,1]+num[5,2]+num[6,3]+num[7,0])\
        +0.5*(1.0+1.0*1j)*(num[5,0]+num[6,1]+num[7,2]+num[4,3]+num[1,4]+num[2,5]+num[3,6]+num[0,7])\
        )
    list_enekxky, list_veckxky, list_intkxky = \
        calc_k_ene(Lx,Ly,Lsub,Lspin,LRBZsize,BCx,BCy,list_kx,list_ky,U,n,m,Qx,Qy)
...
#    print("list_sorted_intkxky",list_sorted_intkxky)
    ...
    for i in range(Lsub*Lspin):
        for j in range(Lsub*Lspin):
            num[i,j] = np.inner(np.conjugate(list_sorted_veckxky[0:numel,i]),list_sorted_veckxky[0:numel,j])/Lx/Ly
#    print("num",num)
...
## spin:  (0,      0,        0,         0,         1,      1,        1,         1        )
    ...
    list_sub = list_ss%Lsub
    list_spin = list_ss//Lsub
...
#
        ...
        ## states with "ss=0,1,2,3" are required at half filling
        ## choose pairs (c_k,c_-k) with ky>0
        if list_ss[i]<Lsub and list_RBZ_ky[list_xy[i]]>0 and list_intkxky[i] not in list_paired:
            ## left ss
            ind = Lsub*Lspin*list_ik2ind[i]+list_ss[i]
            whind = np.where(list_intkxky == ind)[0]
            ## right ss
            indm = Lsub*Lspin*list_mik2ind[i]+list_ss[i]
            whindm = np.where(list_intkxky == indm)[0]
            ## add pairs to list
            list_paired.append(ind)
            list_paired.append(indm)
            print("i list_paired",i,list_paired)
            ##
            print("chosen index",list_intkxky[i])
            print("c_left^dagger  (ss,x,y,ind,wind,ind)",list_ss[i],list_x[i],list_y[i],ind,whind,list_intkxky[whind])
            print("c_right^dagger (ss,x,y,ind,wind,ind)",list_ss[i],list_mx[i],list_my[i],indm,whindm,list_intkxky[whindm])
...
#            print("iky",list_RBZ_iky[list_xy[i]])
            ...
            print("fk",np.outer(list_veckxky[whind],list_veckxky[whindm]))
            print("sum fk",fk[list_x[i],list_y[i]])
            print("----")
    return fk
...
#
    ...
    num[7,0] = 1.0/8.0*m
#
    num[5,0] = 1.0/8.0*np.conjugate(m)
    num[6,1] = 1.0/8.0*np.conjugate(m)
    num[7,2] = 1.0/8.0*np.conjugate(m)
    num[4,3] = 1.0/8.0*np.conjugate(m)
    num[1,4] = 1.0/8.0*np.conjugate(m)
    num[2,5] = 1.0/8.0*np.conjugate(m)
    num[3,6] = 1.0/8.0*np.conjugate(m)
    num[0,7] = 1.0/8.0*np.conjugate(m)
...
## loop for Hartree-Fock approximation
    file = open("dat_2d_triangular_steps",'w')
    for step in range(maxstep):
        ...
        filling, numel, chemipo, totene, gap, shellcond, \
            ...
            calc_mean_field(Lx,Ly,Lsub,Lspin,LRBZsize,BCx,BCy,filling_numer,filling_denom,list_RBZ_kx,list_RBZ_ky,U,num,Qx,Qy)
        if diff < diffeps:
            file.write("{} {} {}\n".format(step,num,diff))
            break
...
## calculate shifted energy of Hartree-Fock approximation
    eneU = (U*(0.25*n**2-np.abs(m)**2)).real
...
## calculate fk, fij
    fk = calc_fk(Lx,Ly,Lsub,Lspin,xshift,yshift,numel,list_intkxky,list_veckxky,list_RBZ_kx,list_RBZ_ky,\
        list_RBZ_ikx,list_RBZ_iky,list_RBZ_mikx,list_RBZ_miky,list_RBZ_ik,list_RBZ_mik,list_RBZ_ind,list_RBZ_ik2ind,list_kx,list_ky)
    fiyixjyjx = calc_fiyixjyjx(list_kx,list_ky,Lx,Ly,Lsub,Lspin,fk,Qx,Qy)
    fiyixjyjx01, fiyixjyjx00, fiyixjyjx11 = calc_sym_fiyixjyjx(Lx,Ly,fiyixjyjx)

    print("")
    print("all nonzero fk (fk=0 out of RBZ)")
    print("## order: (c_k_up, c_k+Q_up, c_k+2Q_up, c_k+3Q_up, c_k_dn, c_k+Q_dn, c_k+2Q_dn, c_k+3Q_dn)")
    print("## ss:    (0,      1,        2,         3,         4,      5,        6,         7        )")
    for i in range(Lsub*Lspin):
        for j in range(Lsub*Lspin):
