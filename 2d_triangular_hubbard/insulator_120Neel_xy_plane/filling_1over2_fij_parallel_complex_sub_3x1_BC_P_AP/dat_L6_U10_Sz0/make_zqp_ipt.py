...
def ene(kx,ky,U,n,m,Qx,Qy):
    ...
    def loc_ham(kx,ky,U,n,m,Qx,Qy):
        e1 = tb_ene(kx,ky)+0.5*U*n
        e2 = -U*m
        e3 = np.conjugate(e2)
        e4 = tb_ene(kx+Qx,ky+Qy)+0.5*U*n
        return np.array([[e1,e2],[e3,e4]])
    ## order: (c_k_up, c_k+Q_dn,  c_k+Q_up, c_k-Q_dn,  c_k-Q_up, c_k_dn)
    ## k+2Q \equiv k-Q (mod 2*pi)
    ham1 = loc_ham(kx,ky,U,n,m,Qx,Qy)
    ham2 = loc_ham(kx+Qx,ky+Qy,U,n,m,Qx,Qy)
    ham3 = loc_ham(kx-Qx,ky-Qy,U,n,m,Qx,Qy)
#    print("ham1",ham1)
#    print("ham2",ham2)
#    print("ham3",ham3)
    ene1, vec1 = np.linalg.eigh(ham1)
    ene2, vec2 = np.linalg.eigh(ham2)
    ene3, vec3 = np.linalg.eigh(ham3)
    ene0 = np.concatenate([ene1,ene2,ene3])
    vec1pad0 = np.append(np.append(vec1,np.array([[0,0],[0,0]]),axis=1),np.array([[0,0],[0,0]]),axis=1)
    vec2pad0 = np.append(np.array([[0,0],[0,0]]),np.append(vec2,np.array([[0,0],[0,0]]),axis=1),axis=1)
    vec3pad0 = np.append(np.array([[0,0],[0,0]]),np.append(np.array([[0,0],[0,0]]),vec3,axis=1),axis=1)
    vec0 = np.concatenate([vec1pad0,vec2pad0,vec3pad0])
...
def calc_fk(Lx,Ly,Lsub,Lspin,xshift,yshift,numel,list_intkxky,list_veckxky,list_RBZ_kx,list_RBZ_ky,\
    ...
    list_sub = ((list_ss+1)%(Lsub*Lspin))//Lspin
    list_spin = list_ss%Lspin
...
#
        ...
        ## states with "ss=0,2,4" are required at half filling
        ## choose pairs (c_k,c_-k) with ky>0
        if list_spin[i]==0 and list_RBZ_ky[list_xy[i]]>0 and list_intkxky[i] not in list_paired:
            ## left ss
...
## spin:  (0,      1,         0,        1,         0,        1     )
                            for orb1 in range(Lsub*Lspin):
                                sub1 = ((orb1+1)%(Lsub*Lspin))//Lspin
                                spin1 = orb1%Lspin
                                Q1x = Qx*sub1
                                Q1y = Qy*sub1
                                KRi = (kx+Q1x)*ix + (ky+Q1y)*iy
                                for orb2 in range(Lsub*Lspin):
                                    sub2 = ((orb2+1)%(Lsub*Lspin))//Lspin
                                    spin2 = orb2%Lspin
                                    Q2x = Qx*sub2
                                    Q2y = Qy*sub2
...
## calculate fk, fij
    ...
    print("## order: (c_k_up, c_k+Q_dn,  c_k+Q_up, c_k-Q_dn,  c_k-Q_up, c_k_dn)")
    print("## ss:    (0,      1,         2,        3,         4,        5     )")
    for i in range(Lsub*Lspin):
        for j in range(Lsub*Lspin):
